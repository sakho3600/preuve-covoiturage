...
CREATE MATERIALIZED VIEW trip.export AS (
  SELECT
    cpp.operator_id as operator_passenger_id,
    cpd.operator_id as operator_driver_id,
    opp.name as operator_passenger_name,
    opd.name as operator_driver_name,
  ...
  JOIN operator.operators as opp ON opp._id = cpp.operator_id::int
  ...
  JOIN carpool.carpools AS cpd ON cpd.trip_id = cpp.trip_id AND cpd.is_driver = true
  JOIN operator.operators as opd ON opd._id = cpd.operator_id::int
...
CREATE INDEX ON trip.export(operator_passenger_id);
CREATE INDEX ON trip.export(operator_driver_id);
